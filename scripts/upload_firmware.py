...
def get_firmware_hash(format):
	...
	sha256sum_info = grep(sha256sums_file.readlines(), "openwrt-%s-%s-sysupgrade.img.gz" % (args.device_file, format))[0]
...
def transfer_upload(host_provider, format):
	file_name = "openwrt-%s-%s-%s-sysupgrade.img.gz" % (args.openwrt_version, args.device_file, format)
...
bot.send_document(post_channel, document="%s/openwrt-%s-%s-sysupgrade.zip" % (base_dir, args.openwrt_version,
